...
{
  BIO *bio _G_GUARDED_BY_LOCK(ssl_mutex);
  guint8 *dtls_rx _G_GUARDED_BY_LOCK(ssl_mutex);
  guint8 *dtls_tx _G_GUARDED_BY_LOCK(ssl_mutex);
  GMutex ssl_mutex;

  gboolean shutting_down _G_GUARDED_BY_LOCK(ssl_mutex);
...
g_tls_connection_openssl_finalize (GObject *object)
{
  ...
  priv = (GTlsConnectionOpensslPrivate *)g_tls_connection_openssl_get_instance_private (openssl);
      ...
      switch (err_code)
        {
          ...
          case SSL_ERROR_WANT_WRITE:
            ...
            break;
          default:
            io_needed = (GIOCondition)0;
            break;
        }

      status = end_openssl_io (openssl, direction, ssl, ret, err_code, TRUE, error, err_prefix);
...
_openssl_alpn_select_cb (SSL                  *ssl,
                         ...
                         void                 *arg)
{
  GTlsConnectionBase *tls = (GTlsConnectionBase *)arg;
                                       ...
                                       GError                  **error)
{
  ...
  GTlsCertificateFlags errors = G_TLS_CERTIFICATE_NO_FLAGS;
  gboolean is_client = G_IS_TLS_CLIENT_CONNECTION (tls);

  database = g_tls_connection_get_database (G_TLS_CONNECTION (tls));
  if (database)
    {
      errors = (GTlsCertificateFlags)(errors | g_tls_database_verify_chain (database,
                                                                            chain,
    ...
    {
      errors = (GTlsCertificateFlags)(errors | G_TLS_CERTIFICATE_UNKNOWN_CA);
      errors = (GTlsCertificateFlags)(errors | g_tls_certificate_verify (chain, identity, NULL));
    }
                                ...
                                GError                **error)
{
  ...
  if (SSL_version (ssl) >= TLS1_3_VERSION)
    {
      g_tls_connection_openssl_unlock (tls);
...
perform_read (SSL      *ssl,
              gpointer  user_data)
{
  ReadRequest *req = (ReadRequest *)user_data;
                                       ...
                                       GError             **error)
{
  ...
  *nread = 0;
  ...
  if (!priv->dtls_rx)
    priv->dtls_rx = (guint8 *)g_malloc (DTLS_MESSAGE_MAX_SIZE);
...
perform_write (SSL      *ssl,
               gpointer  user_data)
{
  WriteRequest *req = (WriteRequest *)user_data;
                                        ...
                                        GError             **error)
{
  ...
  if (!priv->dtls_tx)
    priv->dtls_tx = (guint8 *)g_malloc (DTLS_MESSAGE_MAX_SIZE);
...
g_tls_connection_openssl_get_connection_from_ssl (SSL *ssl)
{
  ...
  return (GTlsConnectionOpenssl *)SSL_get_ex_data (ssl, data_index);
}
